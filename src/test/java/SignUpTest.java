...
public class SignUpTest extends BaseTest {
    ...
    public void checkInputTypes() {
        ...
        Assert.assertEquals(signupPage.getNameInputField().getAttribute("type"), "text",
                "The email field does not contain attribute type 'text'.");
    ...
    public void displaysErrorsWhenUserAlreadyExist(){
        ...
        Assert.assertTrue(messagePopUpPage.getErrorMessageTextElement().getText().contains("E-mail already exist"),
    ...
    @Test(priority = 4)
    @Description("TC4 - Sign up")
    public void signup(){
        navPage.getSignUpButton().click();
        signupPage.getNameInputField().sendKeys("Jelena Alempijevic");
        signupPage.getEmailInputField().sendKeys("jelena.alempijevic@itbootcamp.rs");
        signupPage.getPasswordInputField().sendKeys("12345");
        signupPage.getPasswordConfirmField().sendKeys("12345");
        signupPage.getSignMeUpButton().click();
        wait.until(ExpectedConditions.urlContains("/home"));
        messagePopUpPage.waitUntilVerifyYourAccountDialogAppears();
